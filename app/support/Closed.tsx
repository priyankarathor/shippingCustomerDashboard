  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        >
          {["Closed", "Resolved", "Auto Closed"].map((s) => (
            <Checkbox key={s} label={s} />
          ))}
          <FooterButtons />
        </Dropdown>
...
function Dropdown({
  ...
  return (
    <div className="relative">
      <button
        onClick={onToggle}
        className="flex items-center gap-3 border rounded-lg px-4 py-2 bg-white text-sm"
      >
        <Calendar size={20} className="text-gray-600" />
        <span>{label}</span>
        <ChevronDown size={20} className="ml-auto text-gray-600" />
      </button>
