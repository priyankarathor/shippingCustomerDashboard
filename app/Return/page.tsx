...
import Pickupsheduled from "./pickupscheduled"; 
import Returnreceived from "./Retuenreceived"; 
  ...
  return (
    <div className="min-h-screen p-6 bg-gray-50 dark:bg-slate-950 dark:text-white mt-10">
      ...
      <div className="flex items-center gap-4 mb-6 bg-white dark:bg-slate-900 p-4 rounded-2xl border dark:border-slate-800">
        ...
        <button
          onClick={() => router.push("/Addreturns")}
      ...
      {/* ================= TAB CONTENT ================= */}
        {activeSection === "Pending Request" && <Pendingrequest search={search} />}
      {activeSection === "New" && <New search={search} />}
    
       {activeSection === "Return Received" && <Returnreceived search={search} />} 
      {activeSection === "In Transit" && <Intransit search={search} />}
      {activeSection === "Pickup Sheduled" && <Pickupsheduled search={search} />}
      {activeSection === "Pending Refund" && <  Pendingrefund search={search} />}
